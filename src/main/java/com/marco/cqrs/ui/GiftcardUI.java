...
public class GiftcardUI extends UI {
    ...
    protected void init(VaadinRequest vaadinRequest) {
        ...
        layout.setHeight(95, Sizeable.Unit.PERCENTAGE);
    ...
    private Panel issuePanel() {
        ...
        submit.addClickListener(evt -> {
            commandGateway.sendAndWait(new IssueCmd(id.getValue(), Long.valueOf(amount.getValue())));
    ...
    private Panel bulkIssuePanel() {
        ...
        submit.addClickListener(evt -> {
            for(int i = 0; i < Integer.parseInt(number.getValue()); i++) {
                ...
                commandGateway.sendAndWait(new IssueCmd(id, Long.valueOf(amount.getValue())));
            }
            Notification.show("Success", Notification.Type.HUMANIZED_MESSAGE);
        });
    ...
    private Panel redeemPanel() {
        ...
        submit.addClickListener(evt -> {
            commandGateway.sendAndWait(new RedeemCmd(id.getValue(), Long.valueOf(amount.getValue())));
            Notification.show("Success", Notification.Type.HUMANIZED_MESSAGE);
        });
